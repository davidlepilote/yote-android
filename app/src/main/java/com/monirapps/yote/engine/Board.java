  ...
  public void playMove(Player player, Player opponent, Player.Move move) {
    switch (move.type) {
      ...
      case JUMP:
        ...
        deadBlots.push(opponentCase.takeBlot());
        if (otherOpponentCase != null) {
          deadBlots.push(otherOpponentCase.takeBlot());
        } else {
          deadBlots.push(opponent.takeBlot());
        }
        break;
    }
  }

  public void unplayMove(Player player, Player opponent, Player.Move move) {
    switch (move.type) {
      case ADD:
        player.addBlot(move.cases[0].takeBlot());
        break;
      case SLIDE:
        move.cases[0].setBlot(move.cases[1].takeBlot());
        break;
      case JUMP:
        final Case startingCase = move.cases[0];
        final Case opponentCase = move.cases[1];
        final Case jumpingCase = move.cases[2];
        final Case otherOpponentCase = move.cases[3];
        startingCase.setBlot(jumpingCase.takeBlot());
        if (otherOpponentCase != null) {
          otherOpponentCase.setBlot(deadBlots.poll());
        } else {
          opponent.addBlot(deadBlots.poll());
        }
        opponentCase.setBlot(deadBlots.poll());
        break;
    }
  }
